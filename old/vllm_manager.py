...
class VLLMManager:
    ...
    def get_tool_parser_for_model(self, model_id: str) -> tuple[str, Optional[str]]:
        ...
        # GPT-OSS models (OpenAI's open models with MXFP4 quantization)
            ...
            # GPT-OSS has built-in tool support via "harmony" mode - don't specify parser
            return None, None  # Let vLLM auto-enable tools for GPT-OSS
    ...
    def start(self, model_id: str, name: Optional[str] = None, max_len: Optional[int] = None, gpu_memory_utilization: float = None, tensor_parallel_size: int = 1, gpu_ids: Optional[str] = None):
        ...
        if tool_parser:
            if tool_parser == "auto":
                # Special case: enable auto tool choice without specifying parser (e.g., GPT-OSS)
                print(f"Auto tool choice enabled (parser auto-detection)")
        ...
        with open(log_file, 'w') as f:
            ...
            if tool_parser:
                if tool_parser == "auto":
                    f.write(f"Tool Calling: Enabled (auto-detection)\n")
                else:
